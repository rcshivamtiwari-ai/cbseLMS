...
import { LayoutDashboard, BookOpen, Code2, Database, ClipboardList, Video, Trophy, Users, Settings, LogOut, ChevronRight, BarChart2, Brain, Cpu } from 'lucide-react'
        ...
        {nav.map(({ href, icon:Icon, label }) => (
          <Link key={href} href={href}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all group ${
              isActive(href) ? 'bg-brand-700 text-white shadow' : 'text-brand-300 hover:bg-brand-800 hover:text-white'
            }`}>
